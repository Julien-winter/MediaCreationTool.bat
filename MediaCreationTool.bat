...
:: - Added Windows 11 24H2 (26100.4349) and 25H2 (26200.7171) support
:: - Enhanced TPM/hardware bypass for 24H2/25H2 with HwReqChk and LabConfig registry keys
:: on upgrade: latest build, on offline install: 11 25H2 26200.7171 / 11 24H2 26100.4349 / 11 23H2 22631.2861 / 11 22H2 22621.1702 / 11 21H2 22000.318 / 22H2 19045.3803 / 21H2 19044.1288 / 21H1 19043.1348 / 20H2 19042.1052
...
goto process ::# windows 11 25H2 - 2025 Update (uses 24H2 CAB, same servicing branch)
...
set "CAB=https://download.microsoft.com/download/b/1/9/b19bd7fd-78c4-4f88-8c40-3e52aee143c2/products_win11_20230510.cab"
...
if not defined WT (set "WT=1") else (set "WT=")
if not defined WT if not defined set for %%s in ("HKCU\Console\MCT") do (
...
::# repack XML into CAB
makecab products.xml products.cab >nul 2>nul || %<%:4f " makecab failed "%>%
...
#:: start monitoring
 :mct while ($true) {
  ...
  #:: using automation to click gui buttons directly due to UpgradeNow and CreateUpgradeMedia actions no longer working in 11 MCT
   if ('Select' -ne $env:PRESET) { try {
     ...
     while ($win.FindAll(5,$bt[0]).Count -le $nr) {if ($mct.HasExited) {break :mct}; sleep -m 200}; $all = $win.FindAll(5,$bt[0])
      if ($all[$id].GetSupportedPatterns() -contains [Windows.Automation.SelectionItemPattern]::Pattern) {$all[$id].GetCurrentPattern([Windows.Automation.SelectionItemPattern]::Pattern).Select()};$all[$all.Count-1].SetFocus(); Enter
   ...
   } catch {write-host -fore Yellow "UI automation warning: $($_.Exception.Message)"} }
  ...
  #:: get target $ISO or $USB from setup state file
   ...
   while (-not $ready) {
     try {[xml]$xml = get-content "$WS\Panther\windlp.state.xml"} catch {write-host -fore Yellow "Waiting for state file..."; sleep 3; continue}
  ...
  #:: [issue #91] pick up $ISO$ content and add it to the root of the created media - place your $OEM$ into $ISO$\sources\$OEM$
   if ($null -eq $env:DEF) {
     pushd -lit "$env:ROOT"
     $f1 = '$ISO$';       if (test-path $f1) {xcopy /CYHERHIQ $f1 "$DIR" >$null; write-host -fore Gray "AddDirs", $f1} ; popd
     pushd -lit "$env:WORK"
  ...
  #:: watch media layout progress (with status)
   ...
   while (-not $ready) {
     try {[xml]$xml = get-content "$WS\Panther\windlp.state.xml"} catch {sleep 3; continue}
     foreach ($t in $xml.WINDLP.TASK) { if ($t.Name -eq $task) { foreach ($a in $t.ACTION) { if ($a.ActionName -eq $action) {
   ...
   #:: watch usb layout progress (with status) if target is $USB
    if ($null -ne $USB) {
      ...
      while (-not $ready) {
        ...
        try {[xml]$xml = get-content "$WS\Panther\windlp.state.xml"} catch {continue}
       foreach ($t in $xml.WINDLP.TASK) { if ($t.Name -eq $task) { foreach ($a in $t.ACTION) { if ($a.ActionName -eq $action) {
         $total = $a.ProgressTotal; $current = $a.ProgressCurrent
         if ($null -ne $total -and 0 -ne $total -and $total -eq $current) {$ready = $true}
  ...
  #:: kill MCT process before temporary iso finish (but keep ISO for Auto ISO preset)
   if ($env:VER -ge 22000 -or 'Auto Upgrade' -eq $env:PRESET) {
     ...
     sleep 3; cmd /d /x /c "dism /cleanup-wim >nul 2>nul"
     if ('Auto ISO' -ne $env:PRESET) {del $ISO -force -ea 0 >$null}
   }
...
reg query "HKLM\Software\Microsoft\Windows NT\CurrentVersion\WinPE">nul 2>nul && (
 for %%s in (sCPU sRAM sSecureBoot sStorage sTPM) do reg add HKLM\SYSTEM\Setup\LabConfig /f /v Bypass%%sCheck /d 1 /t reg_dword
